...
class VisualConfig(object):
    ...
    def __init__(self,
                 ...
                 x_layers=5,
                 r_layers=0):
        self.l_layers = l_layers  # language encoder
        self.x_layers = x_layers  # cross-modality encoder
        self.r_layers = r_layers  # object encoder
...
class VisualFeatEncoder(nn.Module):
    ...
    def forward(self, visn_input, obj_embeds=None):
        feats, boxes = visn_input
        
        if obj_embeds is None:
            obj_embeds = self.visn_fc(feats)
            obj_embeds = self.visn_layer_norm(obj_embeds)
        y = self.box_fc(boxes)
        y = self.box_layer_norm(y)
        output = (obj_embeds + y) / 2.
...
# ****************************************************************************
# modified version for specific downstream tasks
# ****************************************************************************
...
class LXMERTModelIB(BertPreTrainedModel):
    """LXMERT Model for IB."""
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
...
class LXRTEncoderV2(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
                visn_feats, visn_attention_mask=None, obj_embeds=None):
        # Run visual embedding layer
        # Note: Word embedding layer was executed outside this module.
        #       Keep this design to allow loading BERT weights.
        visn_feats = self.visn_fc(visn_feats, obj_embeds=obj_embeds)
        visn_feats_tmp = visn_feats
        
        # before_fusion_global_repr = {}
        # Run language layers
        for layer_module in self.layer:
            lang_feats = layer_module(lang_feats, lang_attention_mask)
        # before_fusion_global_repr[""] = lang_feats[:, 0]
        ...
        return lang_feats, visn_feats, visn_feats_tmp


class LXMERTModelCIB(BertPreTrainedModel):
    """LXMERT Model for CIB."""
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
        # 使用embedding - encoder 的划分
        self.encoder = LXRTEncoderV2(config)
        
        self.pooler = BertPooler(config)
        self.apply(self.init_bert_weights)
    
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                inputs_embeds=None, obj_embeds=None,
                visual_feats=None, visual_attention_mask=None):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
        ...
        # Run LXRT backbone
        # v2
        lang_feats, _, visual_feats = self.encoder(
            embedding_output,
            extended_attention_mask,
            visn_feats=visual_feats,
        ...
        return {
            "pooled_output": self.pooler(lang_feats),
            "local_linguistic_embedding": embedding_output,
            "local_visual_embedding": visual_feats,
            "global_representation": lang_feats[:, 0],
        }


class LXRTEncoderV3(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Obj-level image embedding layer
        self.visn_fc = VisualFeatEncoder(config)
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        lang_feats_tmp.append(lang_feats)
        visn_feats_tmp.append(visn_feats)
        return lang_feats_tmp, visn_feats_tmp


class BertPoolerv2(nn.Module):
    def __init__(self, config):
        super(BertPoolerv2, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.activation = nn.Tanh()
    ...
    def forward(self, hidden_states):
        ...
        # first_token_tensor = hidden_states[:, 0]
        # pooled_output = self.dense(first_token_tensor)
        pooled_output = self.activation(self.dense(hidden_states))
        return pooled_output
    

class LXMERTModelCIBv2(BertPreTrainedModel):
    """LXMERT Model for CIB."""
    ...
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddings(config)
        # 使用fusion前后划分
        self.encoder = LXRTEncoderV3(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        # Positional Word Embeddings
        embedding_output = self.embeddings(  # [bs, 20, 768]
            input_ids, token_type_ids, inputs_embeds)
        
        # Run LXRT backbone
        lang_feats_tmp, visn_feats_tmp = self.encoder(
            embedding_output,
            extended_attention_mask,
            visn_feats=visual_feats,
            obj_embeds=obj_embeds,
            visn_attention_mask=extended_visual_attention_mask)
        pooled_output = self.pooler(lang_feats_tmp[-1])
        # pooled_output = (pooled_output * attention_mask.unsqueeze(-1)
        #                  ).sum(1) / attention_mask.sum(1, keepdim=True)
        return {
            "pooled_output": pooled_output,
            "l_embeds": lang_feats_tmp,
            "v_embeds": visn_feats_tmp,
        }
